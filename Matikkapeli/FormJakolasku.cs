...
{
    public partial class FormJakolasku : Form
    {
        public FormJakolasku()
        {
            InitializeComponent();
        }
        ...
        private void FormJakolasku_Load(object sender, EventArgs e)
        {
            LuoRandomLuvut();
        }
        ...
        {
            float vastaus = luku1 / luku2;

            bool success = float.TryParse(tb_Vastaus.Text, out float vastaus2);
            if (success == false)
            {
                MessageBox.Show("Vastauksen pitää olla luku!", "Huom!");
            }
            else
            {
                kierrokset++;
